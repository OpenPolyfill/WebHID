...
fn create_uhid_device(name: &str, descriptor: &[u8]) -> UhidDevice {
    ...
    // bus 0x03 = USB, matching the default of the webhid-mock CLI.
...
fn uhid_sibling_interfaces_get_distinct_ids_and_open_their_own_interface() {
    ...
    // Opening each enumerated device must select that exact interface. The
    // raw descriptor is the strongest identity proof: the bug opened the
    // first hidapi sibling, which would surface interface A's descriptor
    // for an open of interface B.
