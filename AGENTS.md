...
### Device identity (2026-09-16)

The daemon's `device_id` (u32 FNV-1a) is the hash of a per-logical-interface identity string. On Linux that string is the canonical sysfs path of the HID interface; since every interface of a real USB or Bluetooth device canonicalizes to its own stable parent (`.../1-7:1.2`), real hardware needs nothing else. UHID is special: all virtual devices share one parent (`/sys/devices/virtual/misc/uhid`) and the hid device directory name is `bus:vid:pid.instance`, where the instance suffix is a kernel-wide counter reassigned at every device creation. Hashing the full path would change the id at every mock reconnect, while stripping the suffix (the previous behavior) collapsed concurrent sibling interfaces that share bus:vid:pid into one ambiguous id, so `open(device_id)` could pick the wrong hidraw node (observed with two sayo-style UHID devices at hidraw7/hidraw8). The uhid identity therefore keeps the stable `bus:vid:pid` base and mixes in the raw report descriptor (`crates/webhid-daemon/src/hid.rs`, `identity_string`): a recreated virtual device keeps its descriptor and its id (hotplug continuity and persisted grants survive), siblings with different descriptors get different ids, and siblings sharing every observable attribute are indistinguishable and merge into one identity (deduped at enumeration). Invariant: every enumerated logical HID interface has a unique device_id, and `open(device_id)` resolves to that exact interface; genuine 32-bit hash collisions still fail closed (hidden at enumeration, refused at open). Compat: persisted device ids for UHID devices from before the fix no longer match; affected origins must re-pair those devices once. Real USB/Bluetooth hardware ids are unchanged. Regression tests: pure identity tests in `crates/webhid-daemon/src/tests/hid.rs` and real-device tests in `crates/webhid-daemon/src/tests/uhid_siblings.rs` (require `/dev/uhid` access; the mock's protocol layer is a lib target since this change).
